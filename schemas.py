...
class BulletMetadata(BaseModel):
    ...
    harmful_count: int = Field(0, description="Count of times this bullet has been tagged as harmful")
...
class Bullet(BaseModel):
    ...
    playbook_id: str = Field(description="The id of the playbook the bullet belongs to.")
    content: str = Field(description="Content containing lesson and guidance earned from prior runs")
...
class LearnJob(BaseModel):
    ...
    reflection: "Reflection | None" = Field(None, description="Output from the reflector activity.")
    curation: "Curation | None" = Field(None, description="Output from the curator activity.")
...
class UpdateOperation(Operation):
    ...
    type: Literal["UPDATE"] = Field("UPDATE", description="The type of operation to be performed.")
...
class LearnWorkflowInput(BaseModel):
    ...
    learn_job_id: str | None = Field(None, description="The id of the learn job (optional, will use playbook_id if not provided).")
...
class LearnWorkflowOutput(BaseModel):
    ...
    learn_job: LearnJob = Field(..., description="The job tracking the progress of the workflow.")
...
class EmbeddedPromptInput(BaseModel):
    """Request model for POST /embed_prompt."""
    prompt: str = Field(..., description="The prompt to be embedded.")
    playbook: str = Field(..., description="The playbook content to embed.")
...
class ListPlaybooksResponse(BaseModel):
    ...
    playbooks: list[Playbook] = Field(..., description="All registered playbooks visible to this caller.")


class CreatePlaybookRequest(BaseModel):
    """Request model for POST /playbooks."""
    name: str = Field(..., description="Human-readable name of the playbook.")
    description: str | None = Field(None, description="Optional description of this playbook's purpose/scope.")
...
class GetPlaybookResponse(BaseModel):
    ...
    playbook: Playbook = Field(..., description="The requested playbook, including timestamps and metadata.")
...
class CreateBulletRequest(BaseModel):
    ...
    content: str = Field(..., description="The text content of the new bullet (guidance, heuristic, principle, rule, etc.).")


class CreateBulletResponse(BaseModel):
    """Response model for POST /playbooks/{playbook_id}/bullets."""
    bullet: Bullet = Field(..., description="The newly created bullet with assigned id and zeroed counts.")
...
class UpdateBulletContentResponse(BaseModel):
    ...
    bullet: Bullet = Field(..., description="Bullet after content update, with modified_at bumped.")
...
class RetrieveBulletsRequest(BaseModel):
    ...
    user_message: str = Field(..., description="The task or question the agent is about to solve.")
...
class RetrieveBulletsResponse(BaseModel):
    ...
    retrieved_bullets: list[Bullet] = Field(..., description="Subset of bullets ranked as most relevant to this user_message.")


class StartLearnRequest(BaseModel):
    """Request model for POST /playbooks/{playbook_id}/episodes/learn."""
    user_message: str = Field(..., description="Original user instruction that kicked off this agent run.")
    retrieved_playbook: str = Field(..., description="The exact playbook context (bullets) you gave the agent at generation time.")
    trajectory: str = Field(..., description="Full reasoning trace / tool calls / code / outputs from the run.")
    ground_truth: str | None = Field(None, description="If you know the correct final answer / canonical solution for this run, include it.")
    evaluation: str | None = Field(None, description="Execution feedback such as unit test results, runtime errors, grader output, etc.")
    reflector_additional_instructions: str | None = Field(None, description="Optional tweaks or domain-specific guidance to inject into reflector prompt.")
    curator_additional_instructions: str | None = Field(None, description="Optional tweaks or domain-specific guidance to inject into curator prompt.")


class StartLearnResponse(BaseModel):
    """Response model for POST /playbooks/{playbook_id}/episodes/learn."""
    learn_job_id: str = Field(..., description="Identifier of the launched learn_workflow job.")


class GetLearnJobResponse(BaseModel):
    """Response model for GET /playbooks/{playbook_id}/episodes/learn/{learn_job_id}."""
    learn_job: LearnJob = Field(..., description="Learn job status including reflection/curation if available.")
